...
public class AppConfig {
    ...
    /**
     * => 관심사를 확실하게 분리!
     * AppConfig는 애플리케이션의 실제 동작에 필요한 "구현 객체를 생성(선택)" 한다.
    ...
    // Q. Singleton이 깨지는거 아닐까요?
    // @Bean memberService -> new MemoryMemberRepository()
    // @Bean orderService -> new MemoryMemberRepository()

    // 예상
    // call AppConfig.memberService -> call AppConfig.memberRepository -> call AppConfig.memberRepository
